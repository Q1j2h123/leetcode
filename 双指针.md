...
* 核心思想：遍历数组，若该位置的元素为0，将后续元素向后移一个单位，空出来的位置补0
```c
void duplicateZeros(int* arr, int arrSize) {
    int slow=0,fast=0;
    for(;fast<arrSize;fast++){
        if(arr[fast]==0&&(fast)<arrSize-1){
             for(int i=arrSize-1;i>fast+1;i--){
                arr[i]=arr[i-1];
             }
             arr[fast+1]=0; //空出来的位置补0
             fast++;//跳过刚才补0的位置
        }
    }
...
## 2.双向指针（对向移动）
### 2.1 思想
双向指针：队首一个指针，队尾一个指针。满足条件时，进行操作
#### [125.验证回文串](https://leetcode.cn/problems/valid-palindrome)
* 核心思想：剔除掉不符合题目意思的字符，即将数字字母字符留在字符串中。前后指针同时遍历字符串，若前后指针所指的字符不同，则不是回文串。否则，指针移动，进行下一轮比较。
```c
...
* 核心思想：反转元音字母，可使用首尾指针进行遍历。若首尾指针各所指的字母不是元音字母，则指针分别移动。指针同时停止时（因为两个while是并列的），即为元音字母，进行交换。交换后，指针继续移动。


```c
bool isNotVowel(char c) {
    c = tolower(c); // 统一转小写，避免重复判断大小写
    return c != 'a' && c != 'e' && c != 'i' && c != 'o' && c != 'u';
}
char* reverseVowels(char* s) {
    int i=0,slow=0,fast=strlen(s);
    while(slow<fast){
      while(slow<fast&&isNotVowel(s[slow])){
        slow++;
      }
      while(slow<fast&&isNotVowel(s[fast])) {
        fast--;
        }
       char t=s[slow];s[slow]=s[fast];s[fast]=t;
       slow++,fast--;
    }
    return s;
}
